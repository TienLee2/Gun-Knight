...
namespace MegaWorld
{
    ...
    public class ScaleClamp : TransformComponent
    {
        ...
        {
            if(spawnInfo.scale.x > MaxScale)
            {
                spawnInfo.scale.x = MaxScale;
            }
            else if(spawnInfo.scale.x < MinScale)
            {
                spawnInfo.scale.x = MinScale;
            }

            if(spawnInfo.scale.y > MaxScale)
            {
                spawnInfo.scale.y = MaxScale;
            }
            else if(spawnInfo.scale.y < MinScale)
            {
                spawnInfo.scale.y = MinScale;
            }

            if(spawnInfo.scale.z > MaxScale)
            {
                spawnInfo.scale.z = MaxScale;
            }
            else if(spawnInfo.scale.z < MinScale)
            {
                spawnInfo.scale.z = MinScale;
            }
        }
