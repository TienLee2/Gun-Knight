...
namespace MegaWorld 
{
    ...
    public class HeightFilter : Filter 
    {
        ...
        public void SetMaterial(ComputeShader cs, FilterContext fс, int index)
        {
            if(index == 0)
            {
            ...
            cs.SetFloat("_ClampMinHeight", fс.PainterVariables.terrainUnderCursor.transform.position.y);
            cs.SetFloat("_ClampMaxHeight", fс.PainterVariables.terrainUnderCursor.terrainData.size.y + fс.PainterVariables.terrainUnderCursor.transform.position.y);
