...
{
    [Serializable]
    public class TexturesFilter : Filter 
    {
        ...
        {
            Vector2 currUV = Utility.WorldPointToUV(fc.PainterVariables.RaycastInfo.hitInfo.point, fc.PainterVariables.terrainUnderCursor);

            BrushTransform brushTransform = TerrainPaintUtility.CalculateBrushTransform(fc.PainterVariables.terrainUnderCursor, currUV, fc.PainterVariables.Size, 0.0f);
            ...
            for (int i = 0; i < addTexturesToRenderTextureList.Count; i++)
            {
                ...
                PaintContext localTextureContext = TerrainPaintUtility.BeginPaintTexture(fc.PainterVariables.terrainUnderCursor, brushRect, Terrain.activeTerrain.terrainData.terrainLayers[addTexturesToRenderTextureList[i].terrainProtoId]);
        ...
        public bool IsSyncTerrain(Terrain terrain)
        {
            for (int Id = 0; Id < terrain.terrainData.terrainLayers.Length; Id++)
            {
                ...
                for (int i = 0; i < TerrainTextureList.Count; i++)
                {
                    if (Utility.IsSameTexture(terrain.terrainData.terrainLayers[Id].diffuseTexture, TerrainTextureList[i].texture, false))
                    {
                        find = true;
                        break;
                    }
    ...
    public class TexturesFilterEditor 
    {
		...
		{
			GUIStyle LabelTextForSelectedArea = CustomEditorGUI.GetStyle(StyleName.LabelTextForSelectedArea);
			GUIStyle boxStyle = CustomEditorGUI.GetStyle(StyleName.Box);
