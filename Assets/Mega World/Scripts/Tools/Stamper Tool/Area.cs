    ...
    public class AreaGizmoDrawer
    {
        ...
        static void DrawGizmoForArea(StamperTool stamperTool, GizmoType gizmoType)
        {
            ...
            if(MegaWorldPath.AdvancedSettings.VisualisationSettings.VisualizeOverlapCheckSettings)
            {
			    ...
			    if(stamperTool.Data.SelectedVariables.SelectedProtoQuadroItemList.Count != 0)
			    {
                    Bounds bounds = new Bounds(Camera.current.transform.position, new Vector3(50f, 50f, 50f));
                    OverlapCheckSettings.VisualizeOverlapForQuadroItem(bounds, true);
			    }
            }
        ...
        public static void DrawStamperVisualisationifNecessary(StamperTool stamperTool, float multiplyAlpha)
        {
            ...
            if(stamperTool.Data.SelectedVariables.HasOneSelectedType() == false)
            {
                return;
            }

            RaycastInfo raycastInfo;
            Utility.Raycast(Utility.GetRayForGlobalSpawnVisualisation(stamperTool.Area.Bounds.center, stamperTool.Data.SelectedVariables.SelectedType), 
                out raycastInfo, stamperTool.Data.SelectedVariables.SelectedType.GetCurrentPaintLayers());

            if(raycastInfo.isHit)
            {
                PainterVariables painterVariables = new PainterVariables(stamperTool.Area.Bounds, stamperTool.Area.GetCurrentRaw(), raycastInfo);
                stamperTool.StamperVisualisation.Draw(painterVariables, stamperTool.Data, multiplyAlpha);
            }
        }
